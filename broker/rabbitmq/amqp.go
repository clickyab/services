...
var (
	...
	publishCounter = publishCount{
		mu: sync.Mutex{},
	}
	consumeCounter = consumeCount{
		mu: sync.Mutex{},
	}
)
...
func (rb *Amqp) Publish(in broker.Job) error {
	...
	if err == nil {
		publishCounter.IncrSuccess()
	} else {
		publishCounter.IncrFailed()
	}
...
func (rb *Amqp) JobStatiscs() map[string]map[string]int64 {
	psuc, pfailed := publishCounter.Value()
	csuc, cfailed := consumeCounter.Value()

	st := map[string]map[string]int64{
		"Publish": map[string]int64{
			"success": psuc,
			"failed":  pfailed,
		},
